...
public class SpTracerImpl implements Tracer {
    private static final Logger logger = LoggerFactory.getLogger(SpTracerImpl.class);
    ...
    /**
     * The tracer state is volatile to ensure thread safety when queried through {@link SpTracerImpl#isRunning()} or
     * {@link SpTracerImpl#getState()}, or when updated through one of the lifecycle-effecting synchronized methods
     * {@link SpTracerImpl#start(boolean)}}, {@link SpTracerImpl#pause()}, {@link SpTracerImpl#resume()} or
     * {@link SpTracerImpl#stop()}.
     */
    ...
    SpTracerImpl(ConfigurationRegistry configurationRegistry, Reporter reporter, ObjectPoolFactory poolFactory,
                     TracerServerClient apmServerClient, final String ephemeralId, Future<MetaData> metaData) {
        ...
        tracerConfiguration.getRecordingConfig().addChangeListener(new ConfigurationOption.ChangeListener<Boolean>() {
            ...
            public void onChange(ConfigurationOption<?> configurationOption, Boolean oldValue, Boolean newValue) {
                SpTracerImpl.this.recordingConfigChanged(oldValue, newValue);
            }
        });
    ...
    public TracerServerClient getTracerServerClient() {
        return tracerServerClient;
    }
